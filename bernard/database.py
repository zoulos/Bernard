...
try:
    logger.info(
        "Attempting to establish DB connection to {0[server]} to database {0[database]}".format(
            config.cfg["bernard"]["database"]
        )
    )
...
def check_db_ready():
    if connection.is_connected():
        ...
        logger.critical(
            "Unable to start bot: MySQL/MariaDB is not connected. Halting bot."
        )
        exit()
...
async def check_db_connection():
    ...
    while True:
        if connection.is_connected():
            ...
            logger.critical(
                "Database is no longer reachable! This is a fatal error. Halting bot."
            )
            exit()
