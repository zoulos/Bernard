...
async def hello(ctx):
    await discord.bot.say(
        "Hello {0.message.author.mention}! I am alive and well <:DestiSenpaii:399640604557967380>".format(
            ctx
        )
    )
...
async def isowner(ctx):
    if common.isDiscordBotOwner(ctx.message.author.id):
        await discord.bot.say(
            "I live to please {0.message.author.mention} every way possible ( ͡° ͜ʖ ͡°)".format(
                ctx
            )
        )
...
async def isadmin(ctx):
    if common.isDiscordAdministrator(ctx.message.author):
        await discord.bot.say(
            "Somehow Destiny let you have administrator in here... {0.message.author.mention}".format(
                ctx
            )
        )
...
async def isregulator(ctx):
    if common.isDiscordRegulator(ctx.message.author):
        await discord.bot.say(
            "{0.message.author.mention} is a regulator, and is ready to abuse all powers granted by the admins™.".format(
                ctx
            )
        )
