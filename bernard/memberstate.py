...
async def on_member_join(user):
    ...
    if retdb is not None:
        ...
        await discord.bot.send_message(
            discord.mod_channel(),
            "{0} **Retroactive Ban:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}` REASON: `{2}`)".format(
                common.bernardUTCTimeNow(), user, retdb["reason"]
            ),
        )
    ...
    await discord.bot.send_message(
        discord.mod_channel(),
        "{0} **New User:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}`) **Account Age:** {2} **From:** `{3}`".format(
            common.bernardUTCTimeNow(),
            user,
            common.bernardAccountAgeToFriendly(user),
            invite,
        ),
    )
    ...
    if config.cfg["auditing"]["account_age_min"]["enable"]:
        logger.info(
            "on_member_join() Account age minimum is enabled: Minimum account age required {} minutes.".format(
                config.cfg["auditing"]["account_age_min"]["min_age_required"]
            )
        )
        account_age_minutes = int(
            (datetime.datetime.utcnow().timestamp() - user.created_at.timestamp()) / 60
        )
        ...
        if (
            ...
            if config.cfg["auditing"]["account_age_min"]["enforcement"] == "ban":
                logger.warn(
                    "on_member_join() BANNING ID {0.id} for being too new. Account age was {1} minutes, config is {2} minute age..".format(
                        user,
                        account_age_minutes,
                        config.cfg["auditing"]["account_age_min"]["min_age_required"],
                    )
                ...
                await common.ban_verbose(
                    user,
                    "BOT_BAN_VIA_ACCOUNT_TOONEW - {} min old".format(
                        account_age_minutes
                    ),
                )
            else:
                logger.warn(
                    "on_member_join() KICKING ID {0.id} for being too new. Account age was {1} minutes, config is {2} minute age.".format(
                        user,
                        account_age_minutes,
                        config.cfg["auditing"]["account_age_min"]["min_age_required"],
                    )
...
async def on_member_remove(user):
    ...
    await discord.bot.send_message(
        discord.mod_channel(),
        "{0} **Departing User:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}`)".format(
            common.bernardUTCTimeNow(), user
        ),
    )
...
async def on_member_ban(member):
    ...
    await discord.bot.send_message(
        discord.mod_channel(),
        "{0} **Banned User:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}`)".format(
            common.bernardUTCTimeNow(), member
        ),
    )
...
async def on_member_unban(server, user):
    ...
    await discord.bot.send_message(
        discord.mod_channel(),
        "{0} **Unbanned User:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}`)".format(
            common.bernardUTCTimeNow(), user
        ),
    )
    ...
    if retdb is not None:
        ...
        await discord.bot.send_message(
            discord.mod_channel(),
            "{0} **Retroactive Unban:** {1.mention} (Name:`{1.name}#{1.discriminator}` ID:`{1.id}`)".format(
                common.bernardUTCTimeNow(), user
            ),
        )
        journal.update_journal_event(
...
async def on_member_update(before, after):
    ...
    if before.nick != after.nick:
        if before.nick is None:
            await discord.bot.send_message(
                discord.mod_channel(),
                "{0} **Server Nickname Added:** {1.mention} was `{1.name}` is now `{2.nick}` (ID:`{1.id}`)".format(
                    common.bernardUTCTimeNow(), before, after
                ),
            )
            journal.update_journal_event(
                module=__name__,
            ...
            await discord.bot.send_message(
                discord.mod_channel(),
                "{0} **Server Nickname Removed:** {1.mention} was `{1.nick}` is now `{2.name}` (ID:`{1.id}`)".format(
                    common.bernardUTCTimeNow(), before, after
                ),
            )
            journal.update_journal_event(
            ...
            await discord.bot.send_message(
                discord.mod_channel(),
                "{0} **Server Nickname Changed:** {1.mention} was `{1.nick}` is now `{2.nick}` (ID:`{1.id}`)".format(
                    common.bernardUTCTimeNow(), before, after
                ),
            )
            journal.update_journal_event(
    ...
    if before.name != after.name:
        await discord.bot.send_message(
            discord.mod_channel(),
            "{0} **Discord Username Changed:** {1.mention} was `{1.name}` is now `{2.name}` (ID:`{1.id}`)".format(
                common.bernardUTCTimeNow(), before, after
            ),
        )
        journal.update_journal_event(
            ...
            userid=after.id,
            contents="{0.name} -> {1.name}".format(before, after),
        )
