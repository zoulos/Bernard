...
async def purge_inactive_users():
    ...
    while not discord.bot.is_closed:
        ...
        logger.info(
            "Starting background task purge_inactive_users() - Interval {0}".format(
        ...
        est = await discord.bot.estimate_pruned_members(
            ...
            days=config.cfg["housekeeping"]["purge_inactive_users"]["inactive_days"],
        )
        if est > 0:
            # send the cleanup
            ...
            logger.warn(
                "Purging {0} inactive users via purge_inactive_users() background job".format(
                    pruned
                )
            )
            await discord.bot.send_message(
                discord.mod_channel(),
                "{0} **Pruned Inactive Users:** {1} users removed for inactivity of {2} days".format(
                    common.bernardUTCTimeNow(),
                    pruned,
                    config.cfg["housekeeping"]["purge_inactive_users"]["inactive_days"],
                ),
            )

        logger.info(
            "Sleeping background task purge_inactive_users() - Interval {0}".format(
                config.cfg["housekeeping"]["purge_inactive_users"]["interval"]
            )
        )
        journal.update_journal_job(
            module=__name__, job="purge_inactive_users", start=job_start, result=est
        )
        await asyncio.sleep(
            config.cfg["housekeeping"]["purge_inactive_users"]["interval"]
        )
...
async def journal_events_cleanup():
    ...
    while not discord.bot.is_closed:
        logger.info(
            "Starting background task journal_events_cleanup() - Interval {0}".format(
        ...
        seconds_grace = (
            86400 * config.cfg["housekeeping"]["journal_events_cleanup"]["days_to_keep"]
        )
        db_cutoff = int(utc_now - seconds_grace)
        logger.info(
            "journal_events_cleanup() attempting purge of deleted messages older than {0} seconds ({1}) days".format(
                seconds_grace,
                config.cfg["housekeeping"]["journal_events_cleanup"]["days_to_keep"],
            )
        )
        ...
        if deleted_records_count > 0:
            logger.warn(
                "journal_events_cleanup() purging {0} deleted messages from journal_events".format(
                    deleted_records_count
                )
            )
            await discord.bot.send_message(
                discord.mod_channel(),
                "{0} **Pruned Deleted Messages:** {1} messages removed from journal for being older than {2} days".format(
                    common.bernardUTCTimeNow(),
                    deleted_records_count,
